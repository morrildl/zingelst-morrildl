	...
	<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
		...
		<fo:flow flow-name="xsl-region-body">
			...
			<fo:block-container position="absolute" top="{$top + 0.01 + 0.25}in" left="{$left + 0.01}in"
					    height="0.25in" width="2.48in"
					    display-align="center" background-color="#9999ff">
			    <fo:block>
				Level: <xsl:value-of select="level"/>
			    </fo:block>
			</fo:block-container>
			<fo:block-container position="absolute" top="{$top + 0.01 + 0.25}in" left="{$left + 0.01 + 1.25}in"
					    height="0.25in" width="1.20in"
					    display-align="center" background-color="#9999ff">
			    <fo:block text-align="right">
				<xsl:if test="@type = 'instant'">Fast </xsl:if>
				Mantra
			    </fo:block>
			</fo:block-container>
			<fo:block-container position="absolute" top="{$top + 0.01 + 0.5}in" left="{$left + 0.01}in"
			...
			<fo:block-container position="absolute"
					    top="{$top + 0.01 + 1.75}in" left="{$left + 0.01}in"
					    width="2.48in" height="1.73in"
